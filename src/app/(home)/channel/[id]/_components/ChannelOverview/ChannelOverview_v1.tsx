...
const ChannelOverview_v1 = ({
  ...
  return (
    <section className="w-full shadow-lg rounded-md overflow-hidden">
      ...
      <section
        className={clsx(
          "w-full p-4 flex flex-col sm:flex-row sm:items-center gap-5",
          {
            "items-start text-left": direction === "left",
            "sm:flex-col items-center text-center": direction === "center",
            "sm:flex-row-reverse items-end text-right": direction === "right",
          }
        )}
      >
        <div className="size-full aspect-square rounded-full overflow-hidden max-w-32 sm:max-w-48">
          <Image
            src={banner}
            width={400}
            height={400}
...
export default ChannelOverview_v1;
